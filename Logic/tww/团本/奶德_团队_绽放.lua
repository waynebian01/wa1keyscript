...
local noRejuUnit, noRejuHealth, oneRejuUnit, oneRejuHealth = Skippy.FindRejuvenation()
...
if spell("自然之愈").usable and hasVoidBurstUnit then
    return Skippy.UnitHeal(hasVoidBurstUnit, "自然之愈")
end

if Skippy.GetCount(75) >= 5 then
    if spell("激活").usable and averageHealthPct <= 80 then
        return Skippy.UnitHeal("player", "激活")
    end
...
if Skippy.GetCount(85) >= 5 and spell("自然迅捷").usable and casting ~= "愈合" then
...
if noRejuUnit and noRejuHealth < 85 then
    return Skippy.UnitHeal(noRejuUnit, "回春术")
end

if isKnown(155675) and oneRejuUnit and oneRejuHealth < 70 then
    return Skippy.UnitHeal(oneRejuUnit, "回春术")
end
