...
-- 更新符文类型
function e.RuneType(runeIndex)
    local type = GetRuneType(runeIndex)
    e.Rune[runeIndex].type = type
end

-- 周围没有双疾病的单位数量
function e.PlagueCount()
    local hasCount = 0
    local noCount = 0
    if not Skippy.Nameplate then return noCount, hasCount end
    for unit, data in pairs(Skippy.Nameplate) do
        if data and data.exists and data.creatureID ~= 11 and data.aura and data.maxRange and data.maxRange <= 15 then
