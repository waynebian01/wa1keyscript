...
local curve1 = creatColorCurve(1, 100)
local curve255 = creatColorCurve(255, 255)
local curve100 = creatColorCurve(10, 100)
...
local function test()
    ...
    local currentHealthPercent = calc:GetCurrentHealthPercent()
    local curveHealthPercent = calc:EvaluateCurrentHealthPercent(curve1)
    local _, _, b = curveHealthPercent:GetRGB()
    -- 4. 获取 (生命值 - 治疗吸收)
    -- 在 WoW API 的逻辑中，这通常被称为 "有效生命值" 或基础预测起始点

    print("当前实际生命值:", currentHealthPercent)
    print("当前实际生命值:", b)
end
...
local function updatePlayerCastingInfo()
    if state.casting then
        ...
        if castingDuration then
            local castingDurationColor = castingDuration:EvaluateRemainingDuration(curve1)
            local _, _, b = castingDurationColor:GetRGB()
            creat(fixed_blocks.casting, 1 / 255)
        end
    else
...
local function updatePlayerChannelingInfo()
    if state.channeling then
        ...
        if channelDuration then
            local channelDurationColor = channelDuration:EvaluateRemainingDuration(curve1)
            local _, _, b = channelDurationColor:GetRGB()
            creat(fixed_blocks.channel, 1 / 255)
        end
    else
...
local function updatePlayerHealth()
    state.healthPercent = UnitHealthPercent("player", false, curve1)
...
local function updatePlayerPower()
    state.powerPercent = UnitPowerPercent("player", nil, nil, curve1)
...
local function updateUnitHealthInfo(unit)
    ...
    local healthPercent = UnitHealthPercent(unit, false, curve1)
    ...
    if init and blocks.unit and blocks.unit.HealthPercent and blocks.unit_start and blocks.unit_num then
        ...
        creat(index, obj.healthPercent and obj.healthPercent)
    end
end
