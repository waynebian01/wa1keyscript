...
# 添加父目录到路径，用于导入 utils
_FUYUTSUI_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
if _FUYUTSUI_DIR not in sys.path:
    sys.path.insert(0, _FUYUTSUI_DIR)
...
# 配置路径：指向上级目录的通用 config.yml
CONFIG_PATH = os.path.join(_FUYUTSUI_DIR, "config.yml")

PIXELS_PER_ROW = 200  # 扫描 200 个数据点
...
def _get_spec_config(config, class_id, spec_id):
    """合并 state 与职业专精配置。config 结构：state，以及 5->1、5->3 等。"""
...
def build_state_dict(config, row_data, state_config, class_id=None, spec_id=None):
    ...
    # 从第 start 个数值开始解析 30 个队友，每个队友 5 个参数 -> 放入子字典 result["group"]["1"]..["30"]
    group_config = (state_config or {}).get("group")
    if group_config and isinstance(group_config, dict):
        start = group_config.get("start", 25)
...
def get_info(window_title="魔兽世界"):
    ...
    info = build_state_dict(config, row_data, state_config, class_id, spec_id)
    return info
