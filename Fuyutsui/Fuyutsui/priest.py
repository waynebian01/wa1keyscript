...
import ctypes
from utils import *
from GetPixels import scan_with_adaptive_step, build_state_dict
...
try:
    while True:
        ...
        # 扫描并构建 state_dict
        result = scan_with_adaptive_step()
        if not result:
            state_dict = {}
            spec_name = None
        else:
            row_data = result.get(0, {})
            ...
            state_config = config.get(spec_name) if spec_name else config.get("state")
            state_dict = build_state_dict(config, row_data, state_config, class_id, spec_id)
