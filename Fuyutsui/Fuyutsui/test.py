...
def _run_priest_loop():
    ...
    global _logic_enabled, _state_dict, _spec_name, _last_action
    prev_pressed = False
    last_logic_time = 0.0
    ...
    while True:
        ...
        if spec_name == "戒律":
            ...
            if state_dict.get("引导") > 0:
            ...
            elif state_dict.get("圣光涌动") and no_atonement_unit is not None and no_atonement_pct < 90:
                _last_action = f"施放 快速治疗 on {no_atonement_unit}"
                action_hotkey = get_hotkey(int(no_atonement_unit), "快速治疗")
            elif state_dict.get("圣光涌动") and lowest_unit is not None and lowest_unit_pct < 90:
                ...
                _last_action = "施放 福音"
                action_hotkey = get_hotkey(0, "福音")
            elif lowest_unit is not None and lowest_unit_pct is not None and lowest_unit_pct < 80 and not state_dict.get("虚空之盾"):
                _last_action = f"施放 苦修 on {lowest_unit} pct {lowest_unit_pct}"
