...
local hasVoidBurstUnit = Skippy.GetUnitWithAura("虚空爆炸")
...
if spell("绝望祷言").usable and player and player.percentHealth <= 50 then
...
if state.isCombat then
    ...
    if spell("纯净术").usable and hasVoidBurstUnit then
        return Skippy.UnitHeal(hasVoidBurstUnit, "驱散")
    end
...
if spell("治疗祷言").usable and playerAura("分秒必争") and lowestUnit and count90 >= 5 then
...
if spell("圣言术：灵").usable then
    if count90 >= 5 then
        if not isKnown("祷言之环") then
            return Skippy.UnitHeal("spell", "圣言术：灵")
        end
...
if playerAura("圣洁") and lowestUnit then -- 圣洁,瞬发治疗术或治疗祷言
...
if spell("治疗祷言").usable and lowestUnit and Skippy.GetCount(85) >= 5 then
    if not isKnown("祷言之环") then
        return Skippy.UnitHeal(lowestUnit, "治疗祷言")
    end
...
if spell("恢复").usable and not isKnown("恢复增效") then
    ...
    if noRenewUnit and noRenewHealth <= 85 then
        return Skippy.UnitHeal(noRenewUnit, "恢复")
    end
end
...
if state.isCombat then
    if target.canAttack and inRange then
        ...
        return Skippy.UnitHeal("spell", "上个敌人")
    end
end]]
